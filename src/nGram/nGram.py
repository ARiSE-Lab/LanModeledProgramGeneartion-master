...
def main():
    ...
    for epoch in range(args.nepochs):
        ...
        val(args, epoch, model, criterion, val_loader, optimizer)
...
def train(args, epoch, model, criterion, train_loader, optimizer):
    ...
    print('Training epoch: ', epoch, " loss: ", total_loss.tolist()[0], " ppl:", torch.exp(total_loss/c).tolist()[0])

def val(args, epoch, model, criterion, val_loader, optimizer):
    model.eval()
    total_loss = torch.Tensor([0])
    c= 0;
    for batch_idx, (context, target) in enumerate(val_loader):
        ...
        loss = criterion(log_probs, target.squeeze())
        total_loss += loss.data.cpu()
    print('Validation epoch: ', epoch, " loss: ", total_loss.tolist()[0], " ppl:", torch.exp(total_loss/c).tolist()[0])
