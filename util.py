...
def instances_to_tensors(instances, dictionary, num_sentences=1):
    ...
    labels = torch.LongTensor(len(instances), max_sent_length)
    for i in range(len(instances)):
        ...
        # print (instances[i].target)
        labels[i] = sentence_to_tensor(instances[i].target, max_sent_length, dictionary)

    return Variable(all_sentences1),  Variable(labels.view(-1))
...
def evaluate(data_source, model, dictionary, bptt, criterion, testF, epoch):
    ...
    for batch_no, batch in enumerate(data_source):
        data, targets = instances_to_tensors(batch, dictionary)
        if args.cuda:
            data = data.cuda()
    ...
    ppl = torch.exp(total_loss / len(data_source))[0]
    print('Validation epoch: ', epoch, " loss: ", total_loss[0] / len(data_source), " ppl:", ppl)
    testF.write('{}, {}, {}\n'.format(epoch, total_loss[0] / len(data_source), ppl))
    testF.flush()
    return ppl
